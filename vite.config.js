/**
 * Vite configuration for Gods Eye Ghost Edition — a cinematic geospatial app.
 *
 ...
 *   7. AIS live — AISStream websocket, or keyless Open Waters snapshot
...
import {
  bboxFromAnchor as openWatersBboxFromAnchor,
  bboxFromView as openWatersBboxFromView,
...
const OPENSKY_AUTH_MODE_DEFAULT = 'anon';
...
const OPENWATERS_AIS_URL = 'https://ais.openwaters.io/v1/vessels';
const OPENWATERS_CACHE_MS = 12_000;
const OPENWATERS_MAX_RESPONSE_BYTES = 24 * 1024 * 1024;
const _openWatersCache = new Map();
const _openWatersInFlight = new Map();
 ...
 * @returns {string} One of the valid mode strings, or the default ('anon').
 */
...
function contactPhotoProxy() {
  const cache = new Map();
  return {
    name: 'contact-photo-proxy',
    configureServer(server) {
      server.middlewares.use('/api/contact-photo', async (req, res) => {
        ...
        if (!/^[0-9a-f]{6}$/.test(hex)) return send(400, { found: false, error: 'invalid hex' });
        if (cache.has(hex)) return send(200, cache.get(hex));
        try {
          const upstream = await fetch(`https://api.planespotters.net/pub/photos/hex/${hex}`, {
            headers: {
              Accept: 'application/json',
              'User-Agent': 'gods-eye-view-contact-photo/1.0',
            },
          ...
          if (!upstream.ok) {
            ...
            cache.set(hex, miss);
            return send(200, miss);
          }
          const json = await upstream.json();
          const photo = Array.isArray(json?.photos) ? json.photos[0] : null;
          const url = photo?.thumbnail?.src || photo?.thumbnail_large?.src || null;
          const payload = {
            found: Boolean(url),
            url,
...
async function serveAdsbLolPointFallback(req, res, requestedMode, reason) {
  ...
  res.writeHead(200, {
    ...
    'X-Flight-Coverage': `${ADSBLOL_POINT_RADIUS_NM}nm live ADS-B`,
 ...
 *   - 'anon'   (default) — no credentials; worldwide OpenSky when the IP is allowed
    ...
    configureServer(server) {
      server.middlewares.use('/api/opensky', async (req, res) => {
        try {
          ...
          // OAuth rejected or missing: anonymous OpenSky is still live worldwide.
          if ((upstream.status === 401 || upstream.status === 403) && usedMode !== 'anon') {
            upstream = await fetch('https://opensky-network.org/api/states/all?extended=1', {
              headers: { Accept: 'application/json' },
            });
            usedMode = 'anon';
            reason = 'auth_rejected_anonymous';
          }
...
const DEFAULT_TFL_MAX_SOURCES = 500;
...
const CCTV_SOURCE_FETCH_TIMEOUT_MS = 20 * 1000;
...
async function loadTflSourcesFromOpenData() {
  try {
    ...
    for (const place of places) {
      ...
      const hasVideo = videoUrl.startsWith(TFL_IMAGE_ORIGIN) && videoUrl.endsWith('.mp4');
      ...
      cameras.push({
        ...
        mountHeightM: 8,
        groundElevationM: 15,
        feedType: hasVideo ? 'mp4' : 'image',
        url: hasVideo ? videoUrl : imageUrl,
        snapshotUrl: imageUrl,
...
async function refreshCctvSources() {
  ...
  const skipLive = String(process.env.CCTV_SKIP_LIVE || '').trim() === '1';
  // Always pull the public Austin / Caltrans / TfL packs unless explicitly skipped.
  const needsLiveSources = !skipLive;
  ...
  // TfL first so the global cap cannot drop UK cameras.
  const merged = [...fromTfl, ...fromAustin, ...fromCaltrans, ...fromFile, ...fromEnv];
      ...
      server.middlewares.use('/api/cctv', async (req, res) => {
        try {
          ...
          setHealth(cameraId, {
            ...
            sourceKind: 'upstream',
            label: source?.provider || 'Configured source',
            message: 'Live frame unavailable',
          });
          res.writeHead(503, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
          res.end(JSON.stringify({ error: 'Live camera frame unavailable' }));
          return;
        } catch (error) {
...
function openWatersBboxFromRequest(req) {
  const incoming = new URL(req?.url || '', 'http://localhost');
  const west = requiredFiniteQueryNumber(incoming.searchParams, 'west');
  const south = requiredFiniteQueryNumber(incoming.searchParams, 'south');
  const east = requiredFiniteQueryNumber(incoming.searchParams, 'east');
  const north = requiredFiniteQueryNumber(incoming.searchParams, 'north');
  if ([west, south, east, north].every(Number.isFinite)) {
    return openWatersBboxFromView(west, south, east, north);
  }
  const latitude = requiredFiniteQueryNumber(incoming.searchParams, 'lat');
  const longitude = requiredFiniteQueryNumber(incoming.searchParams, 'lon');
  const lat = Number.isFinite(latitude) ? latitude : OPENWATERS_DEFAULT_LAT;
  const lon = Number.isFinite(longitude) ? longitude : OPENWATERS_DEFAULT_LON;
  return openWatersBboxFromAnchor(lat, lon);
}

async function pullOpenWatersVessels(url, maxRows) {
  const controller = new AbortController();
  ...
  try {
    ...
    if (!upstream.ok) throw new Error(`upstream HTTP ${upstream.status}`);
    const payload = await readResponseJsonCapped(upstream, OPENWATERS_MAX_RESPONSE_BYTES);
    return normalizeOpenWatersCollection(payload)
      .sort((a, b) => (Number(b.last_position_epoch) || 0) - (Number(a.last_position_epoch) || 0))
      .slice(0, maxRows);
  } finally {
    clearTimeout(timeoutId);
  }
...
async function fetchOpenWatersSnapshot(req, maxRows) {
  const cacheKey = 'global';
  const cached = _openWatersCache.get(cacheKey);
  if (cached && Date.now() - cached.cachedAt < OPENWATERS_CACHE_MS) {
    return { ...cached, cacheStatus: 'HIT' };
  }
  const request = coalesceProxyRequest(_openWatersInFlight, cacheKey, async () => {
    const rows = await pullOpenWatersVessels(OPENWATERS_AIS_URL, maxRows);
    const record = {
      rows,
      cachedAt: Date.now(),
      newestPositionAt: rows[0]?.last_position_UTC || null,
    };
    _openWatersCache.delete(cacheKey);
    _openWatersCache.set(cacheKey, record);
    while (_openWatersCache.size > OPENWATERS_CACHE_MAX) {
      _openWatersCache.delete(_openWatersCache.keys().next().value);
    }
    return record;
  });
  try {
    ...
    if (!request.shared && error?.name !== 'AbortError') {
      console.warn('[Open Waters AIS] global snapshot failed, using view window:', error?.message || error);
    }
    try {
      const bbox = openWatersBboxFromRequest(req);
      const rows = await pullOpenWatersVessels(
        `${OPENWATERS_AIS_URL}?bbox=${encodeURIComponent(formatOpenWatersBbox(bbox))}`,
        maxRows,
      );
      return {
        rows,
      ...
      if (fallbackError?.name !== 'AbortError') {
        console.warn('[Open Waters AIS]', fallbackError?.message || fallbackError);
      }
      return cached ? { ...cached, cacheStatus: 'STALE' } : null;
    }
  }
...
function rememberOpenWatersTracks(rows) {
  for (const row of rows) {
    const epoch = Number(row?.last_position_epoch);
    if (!row?.mmsi || !Number.isFinite(row.lat) || !Number.isFinite(row.lon)) continue;
    appendAisTrackSample(
      row.mmsi,
      row.lat,
      row.lon,
      Number.isFinite(epoch) ? epoch : Math.floor(Date.now() / 1000),
    );
  }
}

/**
 * Vite plugin: live vessel cache.
 *
 ...
 * JSON snapshot to the Cesium layer. Without a key, the worldwide Open Waters
 * REST snapshot is served instead — live AIS at true coordinates, no invented boats.
 */
function aisLiveProxy() {
  function install(middlewares) {
    middlewares.use('/api/ais-live', async (req, res) => {
      try {
        const incoming = new URL(req.url || '', 'http://localhost');
        const hasAisStreamKey = Boolean(process.env.AISSTREAM_API_KEY);
        ...
        if (incoming.pathname === '/track' || incoming.pathname.startsWith('/track/')) {
          ...
          res.end(JSON.stringify({
            ...
            source: hasAisStreamKey
              ? 'AISStream (accumulated since server start)'
              : 'Open Waters AIS (accumulated since server start)',
        ...
        if (!hasAisStreamKey) {
          const snapshot = await fetchOpenWatersSnapshot(req, maxRows);
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
          res.setHeader('Cache-Control', 'no-store');
          if (!snapshot) {
            res.statusCode = 503;
            res.end(JSON.stringify({
              ...
              status: 'down',
              error: 'Live AIS snapshot unavailable',
              refreshing: false,
          ...
          rememberOpenWatersTracks(snapshot.rows);
          res.statusCode = 200;
          res.end(JSON.stringify({
            rows: snapshot.rows,
            source: 'Open Waters AIS',
            ...
            lastMessageAt: snapshot.cachedAt,
          }));
          return;
        }
      ...
      try {
        ...
        const response = await fetch('https://api.openai.com/v1/responses', {
          ...
          body: JSON.stringify({
            ...
            instructions: [
              "Write one concise intelligence-HUD summary for Gods Eye Ghost Edition.",
    ...
    middlewares.use('/api/realtime/token', async (req, res) => {
      ...
      const sessionConfig = {
        session: {
          ...
          instructions: [
            "You are GEV Voice Control, a concise voice controller for a Cesium geospatial app called Gods Eye Ghost Edition.",
...
const GEV_REALTIME_TOOLS = [
  {
    ...
    description: "Fly the Gods Eye camera to a known city, geocoded country/region/city/landmark, or explicit WGS84 coordinate. Countries/cities frame the whole place; landmarks/buildings use close framing.",
    parameters: {
      type: 'object',
  ...
  },
  {
    ...
    description: "Enable or disable one registered Gods Eye data layer.",
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description: "Set the active Gods Eye visual filter/style.",
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
