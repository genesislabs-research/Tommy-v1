...
import { flyToLondon } from './camera.js';
...
import { initGhostEdition } from './ghostEdition.js';
...
import { initInspectDossier } from './inspectDossier.js';
...
async function init() {
  ...
  try {
    ...
    // Google Photorealistic 3D Tiles are optional. Without a key we boot on
    // OpenStreetMap + the Cesium globe so flights, CCTV, and military layers still run.
    ...
    if (googleApiKey) {
      ...
      console.info('[Init] No Google Maps key — using OpenStreetMap globe.');
    }
    ...
    if (googleApiKey) {
      ...
      try {
        ...
        console.warn('[Init] Google 3D Tiles unavailable, falling back to OSM globe:', tileError);
        loaderStatus.textContent = 'Google 3D Tiles unavailable. Continuing on OpenStreetMap...';
        viewer.scene.globe.show = true;
        tileset = null;
      }
    } else {
      loaderStatus.textContent = 'Loading OpenStreetMap globe...';
      viewer.scene.globe.show = true;
    }
    ...
    const mapStackController = new MapStackController(viewer, {
      ...
      cesiumToken,
      initialStack: tileset ? 'photoreal' : 'esri-aerial',
    ...
    await mapStackController.setStack(tileset ? 'photoreal' : 'esri-aerial', { silent: true });

    const hideLoadingScreen = () => {
      if (!loadingScreen || loadingScreen.classList.contains('hidden')) return;
      loadingScreen.classList.add('hidden');
    };
    // Never leave the operator on a black loader if a later step stalls.
    ...
    // If no share link state, fly to London — live TfL cameras actually work there.
    if (!styleManager.hasShareState) {
      loaderStatus.textContent = 'Flying to London...';
      flyToLondon(viewer);
    } else {
    ...
    // Enable after local restore. Doing it before attachDataManager let saved
    // layer prefs (CCTV off by default) abort the in-flight enable, so the
    // globe came up with no cameras.
    if (!styleManager.hasShareState) {
      loaderStatus.textContent = 'Enabling cameras, air traffic, and events...';
      void Promise.resolve(styleManager.layerStateRestorePromise)
        .catch(() => {})
        .then(() => Promise.all([
          dataManager.setEnabled('flights', true, { origin: 'user' }),
          dataManager.setEnabled('military', true, { origin: 'user' }),
          dataManager.setEnabled('ais-live-vessels', true, { origin: 'user' }),
          dataManager.setEnabled('cctv', true, { origin: 'user' }),
          dataManager.setEnabled('military-installations', true, { origin: 'user' }),
          dataManager.setEnabled('earthquakes', true, { origin: 'user' }),
        ]))
        .catch((error) => {
          console.warn('[Init] Live layers failed to enable:', error);
        });
    }
